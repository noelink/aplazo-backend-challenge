...
public class AgeBasedCreditLineCalculator implements CreditLineCalculatorStrategy{
    ...
    @Override
    public Optional<Double> calculateCreditLine(int age) {
        if (age >= 18 && age <= 25) {
            log.info("Age:{} - Approved credit line for $3000", age);
            return Optional.of(Double.valueOf("3000"));
        } else if (age >= 26 && age <= 30) {
            log.info("Age:{} - Approved credit line for $5000", age);
            return Optional.of(Double.valueOf("5000.00"));
        } else if (age >= 31 && age <= 65) {
            log.info("Age:{} - Approved credit line for $8000", age);
            return Optional.of(Double.valueOf("8000.00"));
        } else {
            log.info("Age:{} - Rejected customer request, age not allowed", age);
